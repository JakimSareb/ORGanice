...
class HeaderContent extends PureComponent {
  ...
  // ORG Mode para Eli: cabeceras cifradas con org-crypt (etiqueta :crypt:)
  isCryptHeader(header) {
    const tags = header.getIn(['titleLine', 'tags']);
    return !!tags && tags.includes('crypt');
  }

  isEncryptedCryptHeader(header) {
    return (
      this.isCryptHeader(header) &&
      (header.get('rawDescription') || '').includes('-----BEGIN PGP MESSAGE-----')
    );
  }

  renderCryptControls(header) {
    if (!this.isCryptHeader(header)) return null;
    if (this.isEncryptedCryptHeader(header)) {
      return (
        <div className="eli-crypt-box">
          <i className="fas fa-lock" /> Contenido cifrado{' '}
          <button
            className="btn eli-crypt-box__button"
            onClick={() => this.props.org.decryptCryptHeader(header.get('id'))}
          >
            Descifrar
          </button>
        </div>
    ...
    return (
      <div className="eli-crypt-box eli-crypt-box--open">
        <i className="fas fa-lock-open" /> Se cifrará al guardar{' '}
        <button
          className="btn eli-crypt-box__button"
          onClick={() => this.props.org.encryptCryptHeaders()}
        >
          Cifrar ahora
        </button>
      </div>
