...
// ORG Mode para Eli: `scope` limita dónde se buscan las palabras sueltas:
//   'headers' (por defecto): título de cualquier encabezado
//   'tasks': título, y solo encabezados con palabra clave TODO/DONE/...
//   'text': título + cuerpo del encabezado (texto, listas, tablas...)
export const isMatch = (filterExpr, scope = 'headers') => {
  ...
  return (header) => {
    ...
    const headlineOnly = todoKeyword ? `${todoKeyword} ${rawTitle}` : rawTitle;
    if (scope === 'tasks' && !todoKeyword) return false;
    const headlineText =
      scope === 'text' ? `${headlineOnly}\n${header.get('rawDescription') || ''}` : headlineOnly;
