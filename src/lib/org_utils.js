...
// ORG Mode para Eli: también ficheros cifrados .org.gpg / .org.asc
export const orgFileExtensions = /\.org(_archive)?(\.gpg|\.asc)?$/;
...
export const getBreadcrumbsStringFunction = (allHeaders, path) => {
  ...
  } else if (path.match(orgFileExtensions)) {
    filename = path.substring(path.lastIndexOf('/') + 1).replace(orgFileExtensions, '');
  } else {
