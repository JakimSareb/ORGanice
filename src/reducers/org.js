...
const searchHeaders = ({ searchFilterExpr = [], headersToSearch, path, scope }) => {
  ...
  const searchFilterFunction = isMatch(searchFilterExpr, scope);
...
export const setSearchFilterInformation = (state, action) => {
  ...
  if (searchFilterValid) {
    ...
    let filteredHeaders = searchHeaders({ searchFilterExpr, headersToSearch, path, scope });
...
// ORG Mode para Eli: selección de contextos GTD
const toggleContextFilter = (state, action) => {
  let selected = state.get('contextFilter') || List();
  if (action.clear) {
    selected = List();
  } else if (selected.includes(action.context)) {
    selected = selected.filter((c) => c !== action.context);
  } else {
    selected = selected.push(action.context);
  }
  state = state.set('contextFilter', selected);
